...
def plot_model_performance():
    """Create visualizations showing model weights and performance metrics"""
    ...
    # Plot 3: Lollipop chart instead of pie chart
    ax3 = fig.add_subplot(gs[0, 1])
    
    # Sort model names by weight for better visualization
    sorted_indices = np.argsort([ensemble_metrics['weights'][m] for m in model_names])[::-1]  # Descending order
    sorted_models = [model_names[i] for i in sorted_indices]
    sorted_weights = [ensemble_metrics['weights'][m] for m in sorted_models]
    ...
    for i, (model, weight, color) in enumerate(zip(sorted_models, sorted_weights, sorted_colors)):
        # Draw the stem
        ax3.plot([0, weight], [i, i], color='gray', linestyle='-', linewidth=1.5, zorder=1)
        # Draw the circle
        ax3.scatter(weight, i, s=weight*300, color=color, alpha=0.8, edgecolor='white', linewidth=1, zorder=2)
        # Add labels
        percentage = (weight / total_weight) * 100
        ax3.annotate(f'{model}: {weight:.3f} ({percentage:.1f}%)', 
                    xy=(weight, i),
                    xytext=(5, 0),  # 5 points horizontal offset
                    textcoords="offset points",
                    ha='left', va='center')
    
    # Remove y-axis ticks since we have labels in the plot
    ax3.set_yticks([])
...
def plot_crypto_performance():
    """Create visualizations for cryptographic performance metrics"""
    ...
    # Check if overhead_bytes exists and is non-empty
    if isinstance(overhead_bytes, list) and len(overhead_bytes) > 0:
        ...
        # Just show the average overhead as a text annotation
        avg_overhead = crypto_metrics['encryption'].get('average_overhead', 0)
        if avg_overhead > 0:
            ax3.text(0.5, 0.5, f"Average Encryption Overhead: {avg_overhead:.2f} bytes", 
                    ha='center', va='center', fontsize=16, 
...
def plot_combined_feature_importance():
    """Create a visualization that combines feature importance across models"""
...
def plot_intrusion_timeline():
    """Create visualization of intrusion detection timeline"""
...
def plot_traffic_analysis():
    """Create comprehensive traffic analysis visualization dashboard"""
...
def plot_connection_state_analysis():
    """Create a professionally enhanced visualization without the correlation plot and with fixed percentage display"""
    print("Creating enhanced connection state analysis (3-panel version)...")
    ...
    # Use a meaningful color scheme - different color for each state
    state_colors = {
        'ECO': '#3498db',  # Blue
        'PAR': '#2ecc71',  # Green
        'RST': '#e74c3c',  # Red
    ...
    # Use a clear distinction for traffic types
    traffic_colors = {
        'Normal': '#2ca02c',     # Green
        'Intrusion': '#d62728'   # Red
    }
    ...
    if state_cols:
        ...
        # Create a color list that matches the states in the order they appear
        bar_colors = [state_colors.get(state, '#333333') for state in states]
        
        # Create bar chart with meaningful colors
        bars = ax1.bar(states, counts, color=bar_colors)
        
        # Add value labels with improved positioning and font size
        for bar in bars:
            height = bar.get_height()
        ...
        # Add percentage text box with improved formatting - limited to top 5 states without counts
        percentage_text = "State Distribution:\n"
        for i in range(min(5, len(states))):
            percentage_text += f"{states[i]}: {percentages[i]:.1f}%\n"
        
        # Position the text box in a better location that won't overlap with bars
        ax1.text(0.02, 0.05, percentage_text,
                transform=ax1.transAxes,
                fontsize=14, fontweight='bold',
                bbox=dict(facecolor='white', alpha=0.9, boxstyle='round,pad=0.5', edgecolor='gray'))
        
        ax1.set_title('Connection State Distribution', fontsize=18, fontweight='bold', pad=20)
    ...
    if 'label' in traffic_data.columns and state_cols:
        ...
        # Get top states in the same order as in the first plot for consistency
        ...
        # Create grouped bar chart with meaningful colors
        x = np.arange(len(state_names))
        width = 0.35
        
        ax2.bar(x - width/2, normal_pcts, width, label='Normal', color=traffic_colors['Normal'])
        ax2.bar(x + width/2, intrusion_pcts, width, label='Intrusion', color=traffic_colors['Intrusion'])
        
        # Add value labels with improved visibility
        for i, v in enumerate(normal_pcts):
            ax2.text(i - width/2, v + 0.8, f'{v:.1f}%', ha='center', fontweight='bold', fontsize=12,
                    bbox=dict(facecolor='white', alpha=0.8, boxstyle='round,pad=0.2', edgecolor='none'))
        for i, v in enumerate(intrusion_pcts):
            ax2.text(i + width/2, v + 0.8, f'{v:.1f}%', ha='center', fontweight='bold', fontsize=12,
                    bbox=dict(facecolor='white', alpha=0.8, boxstyle='round,pad=0.2', edgecolor='none'))
        
        ax2.set_title('Connection States by Traffic Type', fontsize=18, fontweight='bold', pad=20)
        ...
        ax2.legend(fontsize=14, loc='upper right', framealpha=0.9, edgecolor='gray')
        
        # Set y-axis to start at 0 with some headroom for labels
        y_max = max(max(normal_pcts), max(intrusion_pcts)) * 1.2
    ...
    # Plot 3: Create a state distribution matrix/heatmap with consistent ordering
    ax3 = fig.add_subplot(gs[1, :])  # Span across both columns
    
    if state_cols and 'label' in traffic_data.columns:
        # Use the same ordering of states as in the top bar charts
        ordered_states = state_sums.index
        
        # Create a matrix of counts for each state by label
        matrix_data = []
        
        # Calculate state percentages for each label
        for label_val, label_name in [(0, 'Normal'), (1, 'Intrusion')]:
            label_data = traffic_data[traffic_data['label'] == label_val]
            
            if len(label_data) > 0:
                row = [label_name]
                for state in ordered_states:
                    # Calculate percentage of this label's traffic with this state
                    pct = label_data[state].sum() / len(label_data) * 100
                    row.append(pct)
                matrix_data.append(row)
        
        if matrix_data:
            # Convert to DataFrame with state names as column headers
            state_names = [s.replace('state_', '') for s in ordered_states]
            df = pd.DataFrame(matrix_data, columns=['Type'] + state_names)
            
            # Set the 'Type' column as index
            df.set_index('Type', inplace=True)
            
            # Create heatmap with improved annotations and color scheme
            heatmap = sns.heatmap(df, annot=True, fmt='.1f', cmap='YlGnBu', ax=ax3, 
                       annot_kws={"size": 14, "weight": "bold"},
                       cbar_kws={"shrink": .8, "label": "Percentage (%)"})
            
            # Increase font size of colorbar
            cbar = heatmap.collections[0].colorbar
            cbar.ax.tick_params(labelsize=12)
            cbar.set_label('Percentage (%)', size=14, weight='bold')
            
            ax3.set_title('State Distribution by Traffic Type (%)', fontsize=18, fontweight='bold', pad=20)
            ...
            ax3.text(0.5, 0.5, "Insufficient data for state matrix", 
                    ha='center', va='center', fontsize=20)
    else:
        ax3.text(0.5, 0.5, "Missing data for state matrix", 
                ha='center', va='center', fontsize=20)
    
    # Add a title for the entire figure
    fig.suptitle('Network Connection State Analysis', fontsize=24, fontweight='bold', y=0.98)
    
    # Save high-resolution output
    plt.savefig(output_dir / 'connection_state_analysis.png', dpi=300)
    plt.close()
    
    print("Enhanced 3-panel visualization created successfully!")

# 7. Create a comprehensive summary dashboard
def create_summary_dashboard():
    """Create a comprehensive dashboard that summarizes key findings"""
    print("Creating summary dashboard...")


     # Define colors dictionary inside the function to avoid scope issues
    colors = {
        'cnn': '#1f77b4',      # Blue
        'lstm': '#ff7f0e',     # Orange
        'dnn': '#2ca02c',      # Green
    ...
    ax2 = fig.add_subplot(gs[0, 1])
    operations = ['Key\nOps', 'Decryption','Encryption']
    
    # Convert from seconds to microseconds (µs) for better readability
    times_us = [
        crypto_metrics['key_operations']['average_time'] * 1_000,
        crypto_metrics['decryption']['average_time'] * 1_000,
        crypto_metrics['encryption']['average_time'] * 1_000
    ]
    
    bars = ax2.bar(operations, times_us, color=['#3274A1', '#E1812C', '#3A923A'])
    
    # Add value labels with µs units
    for bar in bars:
        height = bar.get_height()
    ...
    ax2.set_title('Cryptographic Performance', loc ='left', pad = 20)
    ...
    # Use a color palette from blue to purple
    colors = plt.cm.Blues(np.linspace(0.4, 0.8, len(components)))
    
    # Sort components by size for better visualization
    sorted_components = sorted(zip(components, sizes), key=lambda x: x[1], reverse=True)
    components = [c[0] for c in sorted_components]
    sizes = [c[1] for c in sorted_components]
    
    # Calculate percentages for labels
    percentages = [size / total_size * 100 for size in sizes]
    
    # Create a horizontal bar showing all components
    left = 0
    for i, (component, size, pct) in enumerate(zip(components, sizes, percentages)):
        bar = ax4.barh(y_pos, size, height=bar_height, left=left, 
                     color=colors[i], edgecolor='white', linewidth=1)
        
        # Add component label in the middle of each segment
        if size / total_size > 0.1:  # Only add text for segments that are large enough
            middle = left + size/2
            ax4.text(middle, y_pos, f"{component}\n{pct:.1f}%", 
                   ha='center', va='center', fontweight='bold', color='black')
        
        left += size
    
    # Customize the plot
    ax4.set_yticks([])  # Hide y-axis ticks since it's just one bar
    ax4.set_xlim(0, total_size * 1.05)  # Add some padding
    ax4.set_xlabel('Size (bytes)')
    ax4.set_title('ECIES Component Sizes')
    
    # Add a legend for smaller components
    small_components = [comp for comp, size in zip(components, sizes) if size / total_size <= 0.1]
    if small_components:
        small_pcts = [pct for pct, size in zip(percentages, sizes) if size / total_size <= 0.1]
        small_colors = [color for color, size in zip(colors, sizes) if size / total_size <= 0.1]
        handles = [plt.Rectangle((0, 0), 1, 1, color=color) for color in small_colors]
        labels = [f"{comp} ({pct:.1f}%)" for comp, pct in zip(small_components, small_pcts)]
        ax4.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, -0.15))
    
    # Add annotation with total overhead
    ax4.text(0.05, 0.05, f"Total Overhead: {total_size} bytes",
    ...
    if probabilities:
        ...
        # Add stats annotation
        stats_text = (f"Detections: {len(probabilities)}\n"
              f"Mean: {mean_prob:.4f}\n"
              f"Above Threshold: {sum(p >= threshold for p in probabilities)}")

        ax5.annotate(stats_text, xy=(0.65, 0.85), xycoords='axes fraction',
             fontsize=12, fontweight='bold',
    ...
    ax5.set_title('Detection  Distribution', loc = 'left')
    ...
    if 'label' in traffic_data.columns and 'sbytes' in traffic_data.columns and 'dbytes' in traffic_data.columns:
        ...
        ax8.set_title('Traffic Volume Comparison',  pad = 20)
...
def plot_hr_diagram():
    """
    Create an improved HR diagram-like visualization for model performance metrics.
    
    Similar to HR diagrams in astronomy, this plots one performance metric against another
    to categorize models in different quadrants, with enhanced visibility and clarity.
    """
    print("Creating enhanced HR diagram for model performance...")
    ...
    for model in model_names:
        if model in ensemble_metrics['model_performance']:
            ...
            if 'predictions' in perf and 'mean' in perf['predictions']:
                prediction_mean = perf['predictions']['mean']
                prediction_std = perf['predictions'].get('std', 0.05)
                
                performance_metrics[model] = {
                    'confidence': prediction_mean,  # x-axis: "temperature" analog
                    'weight': weight,               # y-axis: "luminosity" analog
                    'std': prediction_std,          # size: variability
                    'color': colors[model]          # color by model type
                }
    
    # Main plot: HR-like diagram
    ax = fig.add_subplot(111)
    
    # Plot each model as a point with improved sizing
    for model, metrics in performance_metrics.items():
        # Improved sizing algorithm with minimum size and logarithmic scaling
        # This ensures all points are visible while maintaining relative size differences
        min_size = 300  # Minimum circle size for visibility
        max_size = 3000  # Maximum circle size to prevent oversized circles
        
        # Use logarithmic scaling to better handle wide ranges of std values
        if metrics['std'] > 0:
            point_size = min_size + (np.log1p(metrics['std'] * 100) * 800)
        else:
            point_size = min_size
            
        # Cap at maximum size
        point_size = min(point_size, max_size)
        
        # Plot with improved visibility
        scatter = ax.scatter(
            metrics['confidence'],  # x-axis: prediction confidence
            metrics['weight'],      # y-axis: model weight
            s=point_size,           # size: improved scaling of standard deviation
            color=metrics['color'], # color: by model type
        ...
        # Improved label placement with offset adjusted based on point size
        # Calculate offset based on point radius (sqrt of area / π)
        offset = np.sqrt(point_size / np.pi) * 0.7
        
        # Add model name as label with improved positioning
        ax.annotate(
            model.upper(),
            xy=(metrics['confidence'], metrics['weight']),
            xytext=(offset, offset),  # Dynamic offset based on point size
            textcoords='offset points',
            fontsize=14,
            fontweight='bold',
            bbox=dict(facecolor='white', alpha=0.8, edgecolor='gray', boxstyle='round,pad=0.2'),
            zorder=4  # Ensure labels are above all other elements
        )
    
    # Add quadrant lines (dividing at median values)
    if performance_metrics:
        # Get actual values for calculations
        confidences = [m['confidence'] for m in performance_metrics.values()]
        weights = [m['weight'] for m in performance_metrics.values()]
        
        # Calculate visible range and effective median
        # This ensures more balanced quadrants
        confidence_min, confidence_max = min(confidences), max(confidences)
        weight_min, weight_max = min(weights), max(weights)
        
        # Calculate effective median based on visible range
        # This works better than simple median if data is skewed
        median_confidence = confidence_min + (confidence_max - confidence_min) / 2
        median_weight = weight_min + (weight_max - weight_min) / 2
        
        # Draw quadrant lines
        ax.axhline(y=median_weight, color='gray', linestyle='--', alpha=0.7, zorder=1)
        ax.axvline(x=median_confidence, color='gray', linestyle='--', alpha=0.7, zorder=1)
        
        # Calculate positions for quadrant labels based on data range
        # This ensures labels are positioned relative to the actual data distribution
        x_left = confidence_min + (median_confidence - confidence_min) / 2
        x_right = median_confidence + (confidence_max - median_confidence) / 2
        y_top = median_weight + (weight_max - median_weight) / 2
        y_bottom = weight_min + (median_weight - weight_min) / 2
        
        # Add enhanced quadrant labels positioned based on data range
        # Upper left
        ax.text(
            x_left, y_top, 
            "High Weight\nLow Confidence",
    ...
    ax.set_title('Model Performance HR Diagram', fontsize=24, fontweight='bold', pad=20)
    ax.set_xlabel('Prediction Confidence (High → Low Uncertainty)', fontsize=18, fontweight='bold')
    ax.set_ylabel('Model Weight in Ensemble (High → More Influential)', fontsize=18, fontweight='bold')
    ...
    if performance_metrics:
        ...
        x_padding = (confidence_max - confidence_min) * 0.15
        y_padding = (weight_max - weight_min) * 0.15
        
        # Set limits with padding
        ax.set_xlim(confidence_min - x_padding, confidence_max + x_padding)
        ax.set_ylim(weight_min - y_padding, weight_max + y_padding)
    
   
    
    # Add size legend for better interpretation
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'model_hr_diagram.png', bbox_inches='tight', dpi=300)
    plt.close()
    
    print("Enhanced HR diagram created successfully!")
...
def main():
    ...
    plot_hr_diagram()  # New HR diagram visualization
