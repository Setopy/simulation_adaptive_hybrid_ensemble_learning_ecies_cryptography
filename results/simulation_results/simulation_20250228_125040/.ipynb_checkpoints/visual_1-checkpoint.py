...
def plot_traffic_analysis():
    ...
    if state_cols:
        ...
        # Plot horizontal bar chart
        bars = ax3.barh(clean_state_names, state_sums.values, 
...
def plot_connection_state_analysis():
    """Create a detailed visualization of connection state distributions"""
    ...
    # Plot 3: Create a state distribution matrix/heatmap
    ax3 = fig.add_subplot(gs[1, 0])
    ...
    if state_cols and 'label' in traffic_data.columns:
        ...
        if matrix_data:
            ...
            state_names = [s.replace('state_', '') for s in state_cols]
            ...
            # Create heatmap
            sns.heatmap(df, annot=True, fmt='.1f', cmap='YlGnBu', ax=ax3)
            ax3.set_title('State Distribution by Traffic Type (%)')
    ...
    # Plot 4: Connection state correlation
    ax4 = fig.add_subplot(gs[1, 1])
    
    if state_cols and len(state_cols) >= 2:
        # Calculate correlation between states
        state_corr = traffic_data[state_cols].corr()
        
        # Create a correlation heatmap
        mask = np.triu(np.ones_like(state_corr, dtype=bool))  # Create mask for upper triangle
        cmap = sns.diverging_palette(230, 20, as_cmap=True)  # Create diverging colormap
        
        # Clean state names for better display
        state_corr.index = [s.replace('state_', '') for s in state_corr.index]
        state_corr.columns = [s.replace('state_', '') for s in state_corr.columns]
        
        # Plot heatmap
        sns.heatmap(state_corr, mask=mask, cmap=cmap, vmax=1, vmin=-1, center=0,
                   square=True, linewidths=.5, cbar_kws={"shrink": .5}, ax=ax4, annot=True, fmt='.2f')
        
        ax4.set_title('Connection State Correlation')
    else:
        ax4.text(0.5, 0.5, "Insufficient state data for correlation", 
                ha='center', va='center', fontsize=20)
...
# 8. HR Diagram for Model Performance
def plot_hr_diagram():
    """
    Create an HR diagram-like visualization for model performance metrics.
    
    Similar to HR diagrams in astronomy, this plots one performance metric against another
    to categorize models in different quadrants.
    """
    print("Creating HR diagram for model performance...")
    fig = plt.figure(figsize=(16, 14))
    
    # Extract model performance metrics
    performance_metrics = {}
    
    # We'll use prediction confidence as our "temperature" analog (x-axis)
    # and model weight as our "luminosity" analog (y-axis)
    ...
    for model, metrics in performance_metrics.items():
        # Scale point size based on standard deviation (more variable = larger)
        # Inverse relationship like in real HR diagram
        # Multiply by scaling factor for visibility
        point_size = metrics['std'] * 5000
        
        ax.scatter(
            metrics['confidence'],  # x-axis: prediction confidence
            metrics['weight'],      # y-axis: model weight
            s=point_size,           # size: standard deviation
    ...
    if performance_metrics:
        median_confidence = np.median([m['confidence'] for m in performance_metrics.values()])
        median_weight = np.median([m['weight'] for m in performance_metrics.values()])
        
        ax.axhline(y=median_weight, color='gray', linestyle='--', alpha=0.7)
        ax.axvline(x=median_confidence, color='gray', linestyle='--', alpha=0.7)
    ...
    # Labels and title
    ax.set_title('Model Performance HR Diagram', fontsize=24, fontweight='bold')
    ...
    if performance_metrics:
        ...
        xmin, xmax = min(confidences), max(confidences)
        ymin, ymax = min(weights), max(weights)
    ...
    ax.text(
        0.5, 0.02,
        "Note: Circle size represents prediction variability (larger = more variable)",
        transform=ax.transAxes,
        ha='center', va='bottom',
        fontsize=14, fontweight='bold',
        bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.5')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'model_hr_diagram.png', bbox_inches='tight')
    plt.close()
